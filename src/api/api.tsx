...
export async function getCarTypeListAndCarcatalogEqualParam(id: String) {//fetch 該車種所有車型號
  const res = await fetch(
...
export async function getSumStatusDetailListMultiplierZeor(  date?: string): Promise<FetcheGetSumStatusList> { //fetch All TrainByRow 
  let formattedDate =dateFormat(date)
  const wait = (ms: number) =>
    new Promise((resolve) => setTimeout(resolve, ms));
  await wait(2000);
...
export async function getSumStatusListEq3Param(trainName: string,date:string) { //fetch TrainByRow 客車專用
  let formattedDate =dateFormat(date)
  const res = await fetch(
...
export async function getSumStatusDetailListMultiplierZeorDeptParamCartypeParamQtypeParam( //fetch 單輛維修詳情 
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/maximo/zz_data?method=getSumStatusDetailList&multiplier=0&dept=${dept}&cartype=${cartype}&qtype=${qtype}&qdate=${date}`,
    {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
...
export async function getFacRepairYearPlan(yearString?: string) { //fetch 機廠檢修圖表
  const wait = (ms: number) =>
    new Promise((resolve) => setTimeout(resolve, ms));
