...
export async function get30DayData(): Promise<Recent30days[]> {
  //fetch current 30 day data
  const query =
    "SELECT ALL_D, EQ11, AVAILABLE, TOTAL, RATION, USUALFLAG FROM ZZ_P_DAILY_REPORT_24_KPI zvdrk WHERE VARCHAR_FORMAT(CURRENT date, 'yyyy-MM-dd') <> ALL_D ORDER BY VARCHAR_FORMAT(ALL_D, 'yyyy-MM-dd') ASC";

  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/maximo/zz_data?method=getResult`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function getSumFailListDaily(
  date?: string
): Promise<FailListData> {
  //fetch 故障通報daily
...
export async function getInvMountListRow(
  date?: string,
  month?: string,
  dept?: string,
  type?: string
...
export async function getOperationSignal() {
  try {
    const query =
      "SELECT EQ2_C, EQ8_C, ASSETNUM, EQ4, SOURCE, DESC FROM ZZ_V_DRIVER ORDER BY SEQ, ASSETNUM ASC, SOURCE ASC";
