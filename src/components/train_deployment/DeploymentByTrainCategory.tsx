...
const DeploymentByTrainCategory: React.FC<MyComponentProps> = ({ trainName, trainCount }) => {
  ...
  return (
    <div
      className={`flex items-start gap-3 p-4 self-stretch w-full flex-[0_0_auto] bg-[#3034380d] rounded overflow-hidden  ${isActive ? 'border-[#397EFF] border-l-4' : ''} flex-col justify-center relative`}
    ...
    >
      ...
      {isActive && <div className="justify-center self-stretch w-full flex-[0_0_auto] flex flex-col items-start relative">
        <SingleBar />
      </div>}
    </div>
  );
